...
class Post(models.Model):
    ...
    #null, blank: 사용자가 내용을 안적더라도 객체 생성될 수 있게 함.
    contents = models.TextField('내용', null=True, blank=True)
    #auto_now_add = True : 객체가 생성되는 시점의 날짜/시간 정보를 기본값으로
    #사용
    pub_data = models.DateField('작성일', auto_now_add=True)
    #User 모델 클래스를 사용할 때 
    #settings.AUTH_USER_MODEL 또는User 모델 클래스로 사용가능 
...
class Image(models.Model):
    ...
    def delete(self, using=None, keep_parents=False):
        ...
        #이유 : image 객체가 지워질 때 image 경로의 파일이 지워지지 않으므로
        #객체가 삭제되는 delete함수 내에 이미지 파일이 삭제되는 코드를 삽입
        self.image.delete()
